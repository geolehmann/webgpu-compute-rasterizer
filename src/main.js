import {
  fullscreenQuadWGSL
} from '../shaders/fullscreenQuad.js';
import {
  computeRasterizerWGSL
} from '../shaders/computeRasterizer.js';
...
async function init() {
  ...
  loader.load(
    ...
    function (object) {
      ...
      const verticesArray = object.children[0].geometry.attributes.position.array;

      const {
        addComputePass,
        outputColorBuffer
      } = createComputePass(presentationSize, device, verticesArray);
      const {
        addFullscreenPass
      } = createFullscreenPass(presentationFormat, device, presentationSize, outputColorBuffer);
...
function createComputePass(presentationSize, device, verticesArray) {
  ...
  const rasterizerPipeline = device.createComputePipeline({
    ...
    compute: {
      module: computeRasterizerModule,
      entryPoint: "main"
    }
  });
